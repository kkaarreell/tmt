...
rlJournalStart
    rlPhaseStartSetup
        ...
        # Skip the plan -> container vs host are not synced so rpms might not be installable
        rlRun 'echo enabled: False >> plans/install/minimal.fmf'
        rlRun "git diff"
    rlPhaseEnd
    ...
    rlPhaseStartCleanup
        ...
        rlRun "su -l -c 'tmt run --id $CONNECT_RUN plans --default finish'"
        rlFileRestore
        rlRun "loginctl terminate-user $USER" 0,1
